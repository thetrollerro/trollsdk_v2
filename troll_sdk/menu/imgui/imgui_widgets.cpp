...
bool ImGui::ButtonEx2( const char* first_label , const char* second_label , ImFont* first_font , ImFont* second_font , const ImVec2& size_arg , ImGuiButtonFlags flags )
{
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	/* breathing animation */ {
		...
		//const ImU32 breathing = GetColorU32(( held && hovered ) ? ImGuiCol_ButtonAnimActive : hovered ? ImGuiCol_ButtonAnimActive : ImGuiCol_ButtonAnimInactive );	

		const ImU32 breathing = GetColorU32( ( held && hovered ) ? ImGuiCol_ButtonAnimActive : hovered ? ImGuiCol_ButtonAnimActive : ImGuiCol_ButtonAnimInactive );

		window->DrawList->AddLine( bb.Min + ImVec2( 5 , 1 ) , bb.Min + ImVec2( 5 , size_arg.y - 1 ) , breathing , 4 );
	}
...
bool ImGui::Button2( const char* first_label , const char* second_label , ImFont* first_font , ImFont* second_font , const ImVec2& size_arg )
{
	return ButtonEx2( first_label , second_label , first_font , second_font , size_arg , 0 );
}
...
bool ImGui::Checkbox( const char* label , bool* v )
{
	...
	const float pad = ImMax( 1.0f , ( float ) ( int ) ( check_sz / 12.0f ) );
	...
	if ( label_size.x > 0.0f ) {
		...
		RenderText( text_bb.GetTL( ) - ImVec2( 0 , 4 ) , label );
		PopStyleColor( );
	}
...
void ImGui::SeparatorEx( ImGuiSeparatorFlags flags )
{
	...
	else if ( flags & ImGuiSeparatorFlags_Horizontal )
	{
		...
		const ImRect bb( ImVec2( x1 , window->DC.CursorPos.y - 5 ) , ImVec2( x2 , window->DC.CursorPos.y + thickness_draw - 5 ) );
...
bool ImGui::BeginCombo( const char* label , const char* preview_value , ImGuiComboFlags flags )
{
	...
	if ( !( flags & ImGuiComboFlags_NoPreview ) )
		window->DrawList->AddRectFilled( frame_bb.Min , ImVec2( value_x2 , frame_bb.Max.y ) , frame_col , style.FrameRounding * 1.5f , ( flags & ImGuiComboFlags_NoArrowButton ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left );
	if ( !( flags & ImGuiComboFlags_NoArrowButton ) )
	{
		ImU32 bg_col = GetColorU32( ( popup_open || hovered ) ? ImGuiCol_ButtonHovered : ImGuiCol_Button );
		ImU32 text_col = GetColorU32( ImGuiCol_Text );
		window->DrawList->AddRectFilled( ImVec2( value_x2 , frame_bb.Min.y ) , frame_bb.Max , bg_col , style.FrameRounding * 1.5f , ( w <= arrow_size ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right );
		if ( value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x )
			RenderArrow( window->DrawList , ImVec2( value_x2 + style.FramePadding.y , frame_bb.Min.y + style.FramePadding.y ) , text_col , ImGuiDir_Down , 1.0f );
	}
	RenderFrameBorder( frame_bb.Min , frame_bb.Max , style.FrameRounding * 1.5f );
	...
	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x + expected_w / 2 - label_size.x / 2 , frame_bb.Min.y - label_size.y * 1.25f ) , label );
...
bool ImGui::Hotkey( const char* label , int* k , const ImVec2& size_arg )
{
	...
	ImVec2 size = ImGui::CalcItemSize( size_arg , ImGui::CalcItemWidth( ) , 0 );
	const ImRect frame_bb( window->DC.CursorPos , window->DC.CursorPos + size );
	const ImRect total_bb( window->DC.CursorPos + ImVec2( label_size.x , 0 ) , frame_bb.Max ); /*+ size + ImVec2( label_size.x > 0 ? label_size.x + 6 : 0 , 0 )*/
	...
	auto key_size = ImGui::CalcTextSize( buf_display );

	//const ImRect clip_rect( frame_bb.Min.x , frame_bb.Min.y , frame_bb.Min.x + size.x , frame_bb.Min.y + size.y ); // Not using frame_bb.Max because we have adjusted size
	//ImGui::RenderTextClipped( frame_bb.Min + style.FramePadding , frame_bb.Max - style.FramePadding , buf_display , NULL , NULL , style.ButtonTextAlign , &clip_rect );


	//ImGui::RenderTextClipped( frame_bb.Min + style.FramePadding - ImVec2( key_size.x + 5 , 10 ) , frame_bb.Max - style.FramePadding , "[" , NULL , NULL , style.ButtonTextAlign , &clip_rect );
	//ImGui::RenderTextClipped( frame_bb.Min + style.FramePadding - ImVec2( 0 , 10 ) , frame_bb.Max - style.FramePadding , buf_display , NULL , NULL , style.ButtonTextAlign , &clip_rect );
	//ImGui::RenderTextClipped( frame_bb.Min + style.FramePadding + ImVec2( key_size.x + 7 , -10 ) , frame_bb.Max - style.FramePadding , "]" , NULL , NULL , style.ButtonTextAlign , &clip_rect );

	ImGui::RenderText( ImVec2( frame_bb.Min.x + style.FramePadding.x + label_size.x * 1.5f - 2 , frame_bb.Min.y - 2 ) , "[" );
	ImGui::RenderText( ImVec2( frame_bb.Min.x + style.FramePadding.x + label_size.x * 1.5f + 4 , frame_bb.Min.y - 2 ) , buf_display );
	ImGui::RenderText( ImVec2( frame_bb.Min.x + style.FramePadding.x + label_size.x * 1.5f + key_size.x + 4 , frame_bb.Min.y - 2 ) , "]" );


	if ( label_size.x > 0 )
		ImGui::RenderText( ImVec2( frame_bb.Min.x , frame_bb.Min.y ) , label );
...
bool ImGui::SliderScalar( const char* label , ImGuiDataType data_type , void* p_data , const void* p_min , const void* p_max , const char* format , float power )
{
	...
	const ImVec2 label_size = CalcTextSize( label , NULL , true );
	const ImRect frame_bb( window->DC.CursorPos , window->DC.CursorPos + ImVec2( w , 12 ) );
	const ImRect total_bb( frame_bb.Min , frame_bb.Max + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f , 0.0f ) );
	...
	if ( !temp_input_is_active )
	{
		...
		if ( focus_requested || clicked || g.NavActivateId == id || g.NavInputId == id )
		{
			...
			{
				temp_input_start = false; //true pt caca
				FocusableItemUnregister( window );
			}
		}
	}
	...
	if ( grab_bb.Max.x > grab_bb.Min.x )
		window->DrawList->AddRectFilled( frame_bb.Min + ImVec2( 1 , 1 ) , grab_bb.Max + ImVec2( 1 , 1 ) , GetColorU32( g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab ) , style.GrabRounding );

	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[ 64 ];
	const char* value_buf_end = value_buf + DataTypeFormatString( value_buf , IM_ARRAYSIZE( value_buf ) , data_type , p_data , format );
	RenderTextClipped( frame_bb.Min - ImVec2( 0 , 2 ) , frame_bb.Max + ImVec2( 0 , 2 ) , value_buf , value_buf_end , NULL , ImVec2( 0.5f , 0.5f ) );
...
bool ImGui::ColorEdit4( const char* label , float col[ 4 ] , ImGuiColorEditFlags flags )
{
	...
	if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) )
	{
		...
		window->DC.CursorPos = ImVec2( pos.x + button_offset_x , pos.y + 2 );

		const ImVec4 col_v4( col[ 0 ] , col[ 1 ] , col[ 2 ] , alpha ? col[ 3 ] : 1.0f );
		if ( ColorButton( "##ColorButton" , col_v4 , flags , ImVec2( 15 , 10 ) ) )
		{
	...
	if ( label != label_display_end && !( flags & ImGuiColorEditFlags_NoLabel ) )
	{
		...
		window->DC.CursorPos = ImVec2( pos.x + text_offset_x , pos.y );
		TextEx( label , label_display_end );
	}
...
bool ImGui::ColorPicker4( const char* label , float col[ 4 ] , ImGuiColorEditFlags flags , const float* ref_col )
{
	...
	{
		PushItemWidth( ( alpha_bar ? bar1_pos_x : bar0_pos_x ) - picker_pos.x );
...
bool ImGui::ColorButton( const char* desc_id , const ImVec4& col , ImGuiColorEditFlags flags , ImVec2 size )
{
	...
	if ( ( flags & ImGuiColorEditFlags_NoBorder ) == 0 )
	{
		if ( g.Style.FrameBorderSize > 0.0f )
			...
			window->DrawList->AddRect( bb.Min , bb.Max , GetColorU32( ImGuiCol_Border ) , rounding ); // Color button are often in need of some sort of border
	}
...
bool ImGui::Selectable( const char* label , bool selected , ImGuiSelectableFlags flags , const ImVec2& size_arg )
{
	...
	ImVec2 size( size_arg.x != 0.0f ? size_arg.x : label_size.x , size_arg.y != 0.0f ? size_arg.y : label_size.y - 8 );
	ImVec2 pos = window->DC.CursorPos;
	pos.y += window->DC.CurrLineTextBaseOffset;
	...
	RenderTextClipped( bb.Min , bb.Max , label , NULL , &label_size , style.SelectableTextAlign , &bb );
