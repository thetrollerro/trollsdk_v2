...
namespace valve_parser
{
	...
	{
	public:
		static bool StrEqu( Str str1, const char* str2 )
		{
			return str1.toString( ) == std::string( str2 );
		}

		static bool StrEquI( Str str1, const char* str2 )
		{
	...
	public:
		...
		Object* ToObject( );
		bool Parse( );
		std::shared_ptr<Node> GetKeyByName( const char* name );
	};
	...
	public:
		...
		std::shared_ptr<Node> BreadthFirstSearch( const char* name, bool caseInsensitive = false )
		{
		...
		std::vector<std::shared_ptr<Node>> BreadthFirstSearchMultiple( const char* name, bool caseInsensitive = false )
		{
